...
export default function DashboardClientLayout({
  ...
  const { isAuthenticated, isLoading, user, isHydrated, setUser } = useAuthStore();
  const router = useRouter();
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [hasSynced, setHasSynced] = useState(false);

  // Sync server-side user to client-side store if needed
  useEffect(() => {
    if (serverUser && !isAuthenticated && !hasSynced) {
      setUser(serverUser);
      setHasSynced(true);
    }
  }, [serverUser, isAuthenticated, hasSynced, setUser]);
  ...
  if (!isHydrated || (isLoading && !serverUser)) {
    return (
  ...
  // If we are definitely not authenticated and not syncing, don't show the dashboard
  if (!isAuthenticated && !serverUser) return null;
